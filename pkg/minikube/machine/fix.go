...
func recreateIfNeeded(api libmachine.API, cc config.ClusterConfig, n config.Node, h *host.Host) (*host.Host, error) {
	...
	if serr != nil || s == state.Stopped || s == state.None {
		...
		if !me || err == constants.ErrMachineMissing {
			out.TJSON(out.CreatingNode, out.Shrug, `{{.driver_name}} "{{.cluster}}" {{.machine_type}} is missing, will recreate.`, out.V{"driver_name": cc.Driver, "cluster": cc.Name, "machine_type": machineType})
			demolish(api, cc, n, h)
	...
	if s == state.Running {
		if !recreated {
			out.TJSON(out.CreatingNode, out.Running, `Updating the running {{.driver_name}} "{{.cluster}}" {{.machine_type}} ...`, out.V{"driver_name": cc.Driver, "cluster": cc.Name, "machine_type": machineType})
		}
		return h, nil
	}

	if !recreated {
		out.TJSON(out.CreatingNode, out.Restarting, `Restarting existing {{.driver_name}} {{.machine_type}} for "{{.cluster}}" ...`, out.V{"driver_name": cc.Driver, "cluster": cc.Name, "machine_type": machineType})
	}
